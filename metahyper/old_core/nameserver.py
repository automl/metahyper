...
from metahyper._networking import nic_name_to_host
...
class NameServer(object):
    ...
    without creating racing conditions. The implementation uses `PYRO4
    <https://pythonhosted.org/Pyro4/nameserver.html>`_ as a backend and this class is
    basically a wrapper.
    """
    ...
    def __init__(self, run_id, working_directory=None, host=None, port=0, nic_name=None):
        ...
        ----------
            ...
            working_directory: str
                path to the working directory of the HPB run to store the nameservers
                credentials. If None, no config file will be written.
            host: str
    ...
    def start(self):
        ...
        thread = threading.Thread(
            target=self.pyro_ns.requestLoop, name="Pyro4 nameserver started"
        )
        thread.start()
