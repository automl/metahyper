...
class BaseIteration(object):
    ...
    runs are processed and (depending on the implementations) determine the further
    development.
    """
    ...
    def __init__(
        ...
        HPB_iter: int
            The current hp_transfer_optimizer iteration index.
    ...
    def add_configuration(self, config=None):
        """
        ...
        if self.is_finished:
            raise RuntimeError(
                "This iteration is finished, you can't add more " "configurations! "
            )

        if self.actual_num_configs[self.stage] == self.num_configs[self.stage]:
            raise RuntimeError(
                f"Can't add another configuration to stage {self.stage:d} in"
                f"iteration {self.HPB_iter:d}."
            )

        config_id = (self.HPB_iter, self.stage, self.actual_num_configs[self.stage])

        self.data[config_id] = Datum(config=config, budget=self.budgets[self.stage])

        self.actual_num_configs[self.stage] += 1

        if not self.result_logger is None:
            self.result_logger.new_config(config_id, config)
    ...
    def register_result(self, job, skip_sanity_checks=False):
        ...
        This function is called from master, don't call this from
        your script.
        """

        if self.is_finished:
            raise RuntimeError(
                "This HB iteration is finished, you can't register more results!"
            )
    ...
    def get_next_run(self):
        ...
        for k, v in self.data.items():
            if v.status == "QUEUED":
                assert (
                    v.budget == self.budgets[self.stage]
    ...
    def _advance_to_next_stage(self, config_ids, losses):
        ...
        raise NotImplementedError(
            "_advance_to_next_stage not implemented for %s" % type(self).__name__
        )
    ...
    def process_results(self):
        ...
        config_ids = list(
            filter(lambda cid: self.data[cid].status == "REVIEW", self.data.keys())
        )
        ...
        for i, a in enumerate(advance):
            if a:
                self.logger.debug(
                    "ITERATION: Advancing config %s to next budget %f"
                    % (config_ids[i], self.budgets[self.stage])
                )
    ...
    def finish_up(self):
        ...
        for _, v in self.data.items():
            assert v.status in [
                ...
                "REVIEW",
                "CRASHED",
            ], "Configuration has not finshed yet!"
            v.status = "COMPLETED"
