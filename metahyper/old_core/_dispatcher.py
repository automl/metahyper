...
class Dispatcher:
    """
    The dispatcher is responsible for assigning tasks to free workers, report results back
    to the master and communicate to the nameserver.
    """
    ...
    def __init__(
        ...
        new_result_callback: function
            function that will be called with a
            `Job instance <hp_transfer_optimizer.core.dispatcher.Job>`_ as argument.
        ...
        if logger is None:
            self.logger = logging.getLogger("hp_transfer_optimizer.dispatcher")
        else:
            self.logger = logger
        ...
        self.pyro_id = "hp_transfer_optimizer.run_%s.dispatcher" % self.run_id
    ...
    def run(self):
        with self.discover_cond:
            ...
            self.logger.info(
                f"DISPATCHER: Pyro daemon running on {self.pyro_daemon.locationStr}"
            )
        ...
        while True:
            ...
            with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
                ...
                for wn, uri in worker_names.items():
                    if not wn in self.worker_pool:
                        w = Worker(wn, uri)
                        if not w.is_alive():
                            self.logger.debug(f"DISPATCHER: skipping dead worker, {wn}")
                            continue
                        update = True
                        self.logger.info(f"DISPATCHER: discovered new worker, {wn}")
            ...
            for wn in all_workers:
                ...
                if not self.worker_pool[wn].is_alive():
                    self.logger.info(f"DISPATCHER: removing dead worker, {wn}")
                    update = True
                    # todo check if there were jobs running on that that need to be
                    # rescheduled

                    current_job = self.worker_pool[wn].runs_job

                    if not current_job is None:
                        self.logger.info(f"Job {str(current_job)} was not completed")
    ...
    def job_runner(self):
        ...
        while True:
            ...
            while self.waiting_jobs.empty() or len(self.idle_workers) == 0:
                self.logger.debug(
                    f"DISPATCHER: jobs to submit = {self.waiting_jobs.qsize():d}, "
                    f"number of idle workers = {len(self.idle_workers):d} -> waiting!"
                )
            ...
            self.logger.debug(f"DISPATCHER: starting job {str(job.id)} on {worker.name}")
            ...
            self.logger.debug(
                f"DISPATCHER: job {str(job.id)} dispatched on {worker.name}"
            )

    def submit_job(self, id_, **kwargs):
        self.logger.debug(f"DISPATCHER: trying to submit job {str(id_)}")
        with self.runner_cond:
            job = Job(id_, **kwargs)
    ...
    @Pyro4.callback
    @Pyro4.oneway
    def register_result(self, id_=None, result=None):
        self.logger.debug(f"DISPATCHER: job {str(id_)} finished")
        with self.runner_cond:
            self.logger.debug("DISPATCHER: register_result: lock acquired")
            # fill in missing information
            job = self.running_jobs[id_]
            job.time_it("finished")
            job.result = result["result"]
            job.exception = result["exception"]

            self.logger.debug(
                f"DISPATCHER: job {str(job.id)} on {job.worker_name} finished"
            )
            self.logger.debug(str(job))

            # delete job
            del self.running_jobs[id_]
