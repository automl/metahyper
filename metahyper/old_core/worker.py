...
class Worker:
    ...
    def __init__(
        ...
        self.worker_id = f"hp_transfer_optimizer.run_{self.run_id}.worker.{socket.gethostname()}.{os.getpid():d}"
    ...
    def _run(self):
        ...
        try:
            with Pyro4.locateNS(host=self.nameserver, port=self.nameserver_port) as ns:
                ...
                dispatchers = ns.list(
                    prefix=f"hp_transfer_optimizer.run_{self.run_id}.dispatcher"
                )
    ...
    def compute(
        self,
        config,
        budget,  # pylint: disable=unused-argument
        ...
        elif isinstance(pipeline_result, numbers.Real):
            return dict(loss=pipeline_result, info=dict())
        else:
            raise ValueError(
                "Needs to be either a number or dict(loss=number, info=dict()"
            )
    ...
    def start_computation(self, callback, id_, *args, **kwargs):
        ...
        try:
            result = {
                "result": self.compute(*args, previous_working_directory=None, **kwargs),
                "exception": None,
            }
        except Exception:
