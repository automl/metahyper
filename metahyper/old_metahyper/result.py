...
from metahyper.old_metahyper.base_iteration import Datum
    ...
    def __init__(self, directory, overwrite=False):
        ...
        overwrite: bool
            ...
            behavior:
                ...
                * True:   The existing files will be overwritten. Potential risk of
        ...
        try:
            ...
            if overwrite:
                ...
                raise FileExistsError(f"The file {self.config_fn} already exists.")
...
def logged_results_to_HBS_result(directory):
    ...
    Returns
    -------
    hp_transfer_optimizer.core.result.Result: :object:
        TODO
    """
    data = {}
    time_ref = float("inf")
    ...
    with open(os.path.join(directory, "configs.json")) as fh:
        for line in fh:
            ...
            data[tuple(config_id)] = Datum(config=config)
    ...
    with open(os.path.join(directory, "results.json")) as fh:
        for line in fh:
            ...
            data[id_].time_stamps[budget] = time_stamps
            data[id_].results[budget] = result
            data[id_].exceptions[budget] = exception
...
class Result:
    ...
    def __init__(self, HB_iteration_data, HB_config, max_valid_runs=None):
        self.max_valid_runs = max_valid_runs
    ...
    def get_incumbent_trajectory(
        self, all_budgets=True, bigger_is_better=True, non_decreasing_budget=True
    ):
        """
        ...
        ----------
            all_budgets: bool
                If set to true all runs (even those not with the largest budget) can be
                the incumbent. Otherwise, only full budget runs are considered
            bigger_is_better:bool
                flag whether an evaluation on a larger budget is always considered better.
                If True, the incumbent might increase for the first evaluations on a
                bigger budget
            non_decreasing_budget: bool
                flag whether the budget of a new incumbent should be at least as big as
                the one for the current incumbent.
        Returns
        -------
            dict:
        ...
        if not all_budgets:
            all_runs = list(
                filter(lambda r: r.budget == self.HB_config["max_budget"], all_runs)
            )
    ...
    def get_runs_by_id(self, config_id):
        """
        returns a list of runs for a given config id_
        ...
        if len(budgets) > 1:
            config_space.add_hyperparameter(
                CS.UniformFloatHyperparameter(
                    "budget", min(budgets), max(budgets), log=True
                )
            )

        hp_names = config_space.get_hyperparameter_names()
        hps = config_space.get_hyperparameters()
        needs_transform = list(
            map(lambda h: isinstance(h, CS.CategoricalHyperparameter), hps)
        )
    ...
    def get_pandas_dataframe(self, budgets=None):
        id2conf = self.get_id2config_mapping()
